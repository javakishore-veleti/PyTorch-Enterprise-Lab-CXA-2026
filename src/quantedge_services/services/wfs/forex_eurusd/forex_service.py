"""ForexEURUSDService — orchestrates all Forex EUR/USD workflow tasks (fully async)."""
...
class ForexEURUSDService:
    ...
    async def download(self, request: ForexDownloadRequest) -> ForexDownloadResponse:
        return await asyncio.to_thread(self._download_task.execute, request)

    async def ingest(self, request: ForexIngestionRequest) -> ForexIngestionResponse:
        ...
        if resp.status == "success":
            ...
            self._session_tensor = None
        return resp

    async def preprocess(self, request: ForexPreprocessRequest) -> ForexPreprocessResponse:
        if self._session_df is None:
            return ForexPreprocessResponse(
                execution_id=request.execution_id,
                total_bars=0, train_bars=0, val_bars=0, test_bars=0,
                nan_filled=0, features=[], processed_parquet_path="",
                status="failed",
                error="No data loaded. Call /admin/foundations/forex/ingest first.",
            )
        resp, tensor = await asyncio.to_thread(
            lambda: self._preprocess_task.execute(request, self._session_df)
        )
        if resp.status == "success":
            self._session_tensor = tensor
        return resp

    async def run_autograd(self, request: ForexAutogradRequest) -> ForexAutogradResponse:
        ...
        return await asyncio.to_thread(
            lambda: self._autograd_task.execute(request, self._session_tensor)
        )

    async def run_tensor_ops(self, request: ForexTensorOpsRequest) -> ForexTensorOpsResponse:
        if self._session_tensor is None:
        ...
        return await asyncio.to_thread(
            lambda: self._tensor_ops_task.execute(request, self._session_tensor)
        )
